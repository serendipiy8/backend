<template>
    <div class="head">
        <el-form ref="searchForm" :model="search">
            <el-form-item>
                <el-input v-model="search.AdminID" placeholder="请输入剧院管理员ID"/>
            </el-form-item>
            <el-form-item>
...
export default {
    ...
    methods: {
        onSubmitSearch() {
            ...
            if (this.search.AdminID == "/") {
                this.$api.selectAdmin().then(res => {
                if (res.data.code == 2000) {
                    eventBus.emit('searchData', res.data.data)
                }
            });
            }else{
                this.$api.searchAdmin(this.search).then(res => {
                ...
                .catch(error => {
                    console.error("Error in searchAdminHandler:", error);
                    this.$message({
                        message: "查询出错",
                        type: "error"
                    });
